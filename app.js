...
function filter(){const w=norm(query).split(' ').filter(Boolean);list=[];for(const it of D.items){if(tab>=0&&!it[2].includes(tab))continue;if(w.length){const n=norm(it[1]);const sz=(it[4]||[]).map(norm);let ok=true;for(const x of w)if(!n.includes(x)&&!sz.includes(x)){ok=false;break}if(!ok)continue}list.push(it)}}
function card(it,idx){const a=document.createElement('a');a.className='card';a.href='product/'+it[0]+'.html';
 ...
 const info=document.createElement('div');info.className='info';const tag=document.createElement('div');tag.className='tag';tag.textContent=D.tabs[it[2][0]];const nm=document.createElement('div');nm.className='nm';nm.textContent=it[1];info.appendChild(tag);info.appendChild(nm);if(it[3]){const sr=document.createElement('div');sr.className='szr';sr.textContent=/sizes$/.test(it[3])?it[3].toUpperCase():'SIZES '+it[3];info.appendChild(sr)}a.appendChild(info);return a}
